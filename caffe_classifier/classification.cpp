...
void CallBackFunc(int event, int x, int y, int flags, void *userdata)
{
   ...
   if (ud->mouseDown && (event == cv::EVENT_MOUSEMOVE))
   {
      cv::Point p1 = ud->upperLeft;

      ud->upperLeft  = cv::Point(std::min(p1.x, x), std::min(p1.y, y));
      ud->lowerRight = cv::Point(std::max(p1.x, x), std::max(p1.y, y));
   }
   else if (event == cv::EVENT_LBUTTONDOWN)
   {
      ud->upperLeft = cv::Point(x, y);
      ud->lowerRight = cv::Point(x, y);
      ud->mouseDown = true;
   }
...
int main(int argc, char** argv) {
  ...
  while (!exit)
  {
	...
	exit = true;
     ...
     {
	imgCrop = img(cv::Rect(ud.upperLeft, ud.lowerRight)).clone();
	std::vector<Prediction> predictions = classifier.Classify(imgCrop);
